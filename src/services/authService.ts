import { getIdToken, signInWithPopup, type User } from "firebase/auth";
...
class AuthService {
  ...
  private loadTokensFromStorage(): void {
    try {
      ...
      if (expiresAt && parseInt(expiresAt) <= Date.now()) {
        ...
        const timeToExpiry = expiresAtMs
    ...
    try {
      ...
      if (result.statusCode !== 200 || !result.data) {
        ...
        throw new Error(
          result.error ||
            result.message ||
            "Backend authentication failed - no data returned",
        );
      }
  ...
  private async createUserDocument(
    ...
    try {
      ...
      console.log(
        "📝 User document creation disabled - would create:",
        userData,
      );
      /*
      ...
      */
      ...
      throw new AuthError(
        ...
        "Không thể tạo tài khoản người dùng. Vui lòng thử lại.",
        error,
      );
    }
  }
      ...
      */
      ...
      if (error?.code === "unavailable") {
        throw new AuthError(
          ...
          "Không thể kết nối tới Firestore. Vui lòng kiểm tra kết nối internet.",
          error,
        );
  ...
  async updateUserRole(uid: string, newRole: UserRole): Promise<void> {
    try {
      ...
      if (response.statusCode !== 200) {
        throw new Error(
          response.error || response.message || "Failed to update user role",
        );
      }
      ...
      console.log(
        "📝 Firestore user role update disabled for uid:",
        uid,
        "role:",
        newRole,
      );
  ...
  async getAllUsers(): Promise<UserProfile[]> {
    try {
      const response =
        await this.makeAuthenticatedRequest<UserProfile[]>("/api/users");
  ...
  async getCurrentUserProfile(): Promise<UserProfile | null> {
    try {
      const response =
        await this.makeAuthenticatedRequest<UserProfile>("/api/auth/profile");
