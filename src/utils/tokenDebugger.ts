...
export class TokenDebugger {
  static logTokenStatus(): void {
    ...
    const minutesUntilExpiry = timeToExpiry ? Math.floor(timeToExpiry / 1000 / 60) : null;
    const secondsUntilExpiry = timeToExpiry ? Math.floor(timeToExpiry / 1000) : null;

    console.group('🔍 Token Status Debug');
    console.log('Current Time:', new Date().toLocaleString());
    console.log('Has Access Token:', !!accessToken, accessToken ? `(${accessToken.substring(0, 20)}...)` : '');
    console.log('Has Refresh Token:', !!refreshToken, refreshToken ? `(${refreshToken.substring(0, 20)}...)` : '');
  ...
  static simulateTokenExpiry(minutesFromNow: number = 4): void {
    ...
    console.log(`⚠️ Token expiry simulated to ${minutesFromNow} minutes from now`);
    console.log('New expiry:', new Date(newExpiryTime).toLocaleString());
    TokenDebugger.logTokenStatus();
  }

  static clearAllTokens(): void {
    const keys = ['accessToken', 'refreshToken', 'tokenExpiresAt', 'userInfo', 'loginTimestamp'];
...
if (typeof window !== 'undefined') {
  ...
  console.log('🔧 TokenDebugger available in console. Try: TokenDebugger.logTokenStatus()');
}
